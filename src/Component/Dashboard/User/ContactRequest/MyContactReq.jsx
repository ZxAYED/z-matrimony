  ...
  const handledelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        AxiosSecure.delete(`/requested/${id}`).then((res) => {
  ...
  return (
    <animated.div style={fadeIn} className="max-w-5xl mx-auto mb-10">
      <Headings
        ...
        subheading={
          "Explore a curated collection of biodatas you've saved in the 'View My Request' section. These profiles have captured your interest and are potential matches for your journey in finding a life partner. Easily manage and keep track of your bookmarked biodatas here. Your saved selections are a personalized gallery, carefully chosen to align with your preferences and aspirations. Dive into this collection to discover profiles that resonate with you, bringing you one step closer to the perfect connection."
        }
      ></Headings>
      <div className="overflow-x-auto">
        <table className="table text-lg table-zebra">
          ...
          {items && items.length > 0 ? (
            items?.map((item, index) => (
              <tbody key={item._id}>
                <tr>
                  ...
                  <td>{item.status}</td>
                  <td>
                    {item.status === "available" ? item.phoneNumber : "Pending"}
                  </td>
                  <td>
            ...
            <tr className="text-center text-xl font-bold text-red-500 ">
              No Saved Requested Contact Data
            </tr>
          )}
        </table>
      </div>
